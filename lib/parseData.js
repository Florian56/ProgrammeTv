...
module.exports = function(xml) {
	return new Promise((resolve, reject) => {
		...
		parser.parseString(xml, (err, result) => {
			...
			var nombreProgrammesTraites = 0;
			
			async.eachSeries(programmes, (programme, callback) => {
				...
				if ((heureDebut == 20 && minutesDebut >= 45) || (heureDebut == 21 && minutesDebut <= 20)) {
					if (programmesTrouves.hasOwnProperty(chaine)) {
						if (heureDebut > programmesTrouves[chaine].heureDebut ||
				...
				nombreProgrammesTraites++;
				
				if (nombreProgrammesTraites == programmes.length) {
